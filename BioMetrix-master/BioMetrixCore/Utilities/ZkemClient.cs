...
namespace BioMetrixCore
{
    public class ZkemClient : IZKEM
    {
        ...
        {
            string filePath = @"H:\NOVITU\Backup\rec.txt";
            string url = "";
            string inputDate = new DateTime(Year, Month, Day, Hour, Minute, Second).ToString();
            using (StreamWriter writer = new StreamWriter(filePath, true))
            {
                writer.WriteLine(" New Entry::" + "Date :"+ inputDate + " UserId: " + EnrollNumber + " ");
                url = "http://www.drbawasakar.com/ras.php?UserId=" + EnrollNumber
                               + "&inputDate=" + inputDate;
    ...
    public bool SSR_GetUserInfo(int dwMachineNumber, string dwEnrollNumber, out string Name, out string Password, out int Privilege, out bool Enabled)
        ...
        {
            return objCZKEM.ReadAOptions(AOption, out AValue);
        }
        ...
        {
            return objCZKEM.ReadCustData(dwMachineNumber, ref CustData);
        }

        public bool ReadDPTInfo(int dwMachineNumber)
        {
            return objCZKEM.ReadDPTInfo(dwMachineNumber);
        }

        public bool ReadFile(int dwMachineNumber, string FileName, string FilePath)
        {
            return objCZKEM.ReadFile(dwMachineNumber, FileName, FilePath);
        }
        ...
        {
            return objCZKEM.SendCMDMsg(dwMachineNumber, Param1, Param2);
        }
